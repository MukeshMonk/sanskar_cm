    ...
    <div class="col-xxl-12 col-md-12">
      <div class="panel panel-default">
        ...
        <div class="panel-body">
          ...
          <? $this->app->htmlBuilder->buildTag("input", array("type"=>"hidden", "value"=>"save_marks"), "act");?>
          ...
          <? $this->app->htmlBuilder->buildTag("input", array("type"=>"hidden", "value"=>$this->app->exam_id),"exam_id");?>
         <?php /*?><div class="panel-heading pull-right" id="dateheaqding">
          ...
          <table class="table table-striped table-bordered"  >
            <thead>
              <tr>
                ...
                <?php /*echo $this->app->max_mark*/ ?>
                <th style="width:15%;" class="sort-column">Subject (<?php echo $this->app->sub_name ?>)( <?php echo $this->app->ex_sub_max_mark ?>)</th>
            ...
            <tbody>
				...
				<tr>					
                    ...
                    <input type="hidden" name="stud_name1[]" value="<?php echo $student['stud_name']; ?>"/>
                    </td>
					<td>
                    <?php 
						$this->app->htmlBuilder->buildTag("select", array("class"=>"select2 select2-hidden-accessible w150 enter_mark_status",  "name"=>"default_status11[]","values"=>$status_array1,"selected"=>$student['default_status']),"");
					?><input type="text" class="form-control edit_marks" data-marks="<?php echo $this->app->ex_sub_max_mark ?>"  data-cmarks="<?php echo $student['marks']; ?>" name="marks1[]" value="<?php echo $student['marks']; ?>"  /></td>
                    ...
                    <td><input type="text" class="form-control" name="grade1[]" value="<?php echo $student['grade']; ?>"  /></td><?php */?>
                     <td><input type="text" class="form-control e-remark" name="remarks1[]" value="<?php echo $student['remarks']; ?>"  /></td>
				</tr>	
				<?php }
				}
				else
				...
				<tr>					
					...
					<td>
                    <?php 
						$this->app->htmlBuilder->buildTag("select", array("class"=>"select2 select2-hidden-accessible w150 enter_mark_status",  "name"=>"default_status11[]","values"=>$status_array1,"selected"=>$selected_vallue),"");
					?><input type="text" class="form-control edit_marks" name="marks1[]" value="<?php echo $remark;?>"  /></td>
          ...
          <?php 
            $date = date('d-m-Y');
            $now = strtotime($date);
            $exam_date1 = $this->app->exam_date;
            $m_sub_date = strtotime($exam_date1);
            if($m_sub_date < $now){ 
          ...
          <div class="panel-footer">          	
          	<button type="submit" class="btn btn-success btn-md" name="save" value="save" <?php echo $disabled ?> >Save</button>          	
          	<button type="submit" class="btn btn-danger btn-md" name="saveandclose" value="savenclose" <?php echo $disabled ?> >Save & Close</button>
          </div>
