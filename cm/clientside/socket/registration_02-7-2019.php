<?php 
	...
	// $total_student=$app->getPostVar('total_student');
	$fiter_condition="";
	if($acd_year_sel!="")
	{
	...
	$ipp=10;
	$sql2="SELECT COUNT(cm_register.id) as tot_std FROM `cm_register` LEFT JOIN cm_classes ON cm_register.enq_class=cm_classes.id	WHERE cm_register.id!=0 AND cm_register.status!='Deleted'".$cond.$fiter_condition.$ord_cond;
	//echo $sql2; exit;
...
$pagination.= "</div>";	
echo $obj_JSON->encode(array("RESULT"=>"OK", "HTML"=>$html,"pagi_btn"=>$pagination,'page_counter'=>$page_counter,"qry"=>$sql,"qry2"=>$total_stu[0]['tot_std']));					
