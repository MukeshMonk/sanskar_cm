<?php 
	...
	$sql="SELECT `cm_representationalattandence`.* FROM `cm_representationalattandence` WHERE id!=0 and status!=2".$cond.$ord_cond;
	...
	while($row = mysql_fetch_array( $rs ))
	{
		...
		$student_id_btn = "";
		$student_id_butn = explode(",", $student_id);
		$countstudent = count($student_id_butn);
		// foreach($student_id_butn  as $student_btn)
		// {
			// $student_id_btn .= "<div><span class='btn btn-primary studentid'>".$student_btn."</span></div>";
		// }
		...
		$action='<div class="btn-group">
										...
										onclick="javascript:edit_representationalattandences(\''.$id_enc.'\')"><i class="glyphicon glyphicon-edit"></i> Edit</a></li>
                                          <li><a href="javascript:void(0);"  onclick="javascript:confrim_del_ajax({\'ac_id\':\''.$id_enc.'\', \'connector\':\'delete_representationalattandences\'})"><i class="glyphicon glyphicon-trash"></i> Delete</a></li>
                                        </ul>
                                      </div>';
