<?php 
	...
	$sql="SELECT `cm_markattandence`.*,
	 ...
	 LEFT JOIN cm_classes ON cm_classes.id = cm_markattandence.class_id
	...
	while($row = mysql_fetch_array( $rs ))
	{
		...
		$action='<div class="btn-group">
										...
										onclick="javascript:edit_markattandences(\''.$id_enc.'\')"><i class="glyphicon glyphicon-edit"></i> Edit</a></li>
                                          <li><a href="javascript:void(0);"  onclick="javascript:confrim_del_ajax({\'ac_id\':\''.$id_enc.'\', \'connector\':\'delete_markattandences\'})"><i class="glyphicon glyphicon-trash"></i> Delete</a></li>
                                        </ul>
                                      </div>';
		$html.="<tr>";
$html.=" <td><label class=\"custom-checkbox-item\">";
		...
		$html.="<td>{$sr}</td>";
		$html.="<td>{$enterattendance}</td>";
		$html.="<td>{$daywisereport}</td>";
		$html.="<td>{$monthwisereport}</td>";
		$html.="<td>{$academic_year}</td>";
        $html.="<td>{$class_name} - {$section_name}</td>";
		$html.="<td>{$date}</td>";
		$html.="<td>{$default_status}</td>";
