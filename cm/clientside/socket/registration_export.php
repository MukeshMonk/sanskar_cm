<?php 
	...
	// $sql="SELECT `cm_register`.* FROM `cm_register` WHERE cm_register.id!=0 ".$cond.$ord_cond;
	 ...
	 if(count($result)>0)
	 {
			...
			//SETUP EXCEL HEADS
			$ExeclHeads=array("Sr1.","Surname","Name","Father Name","Mother Name","Gender","Category","Blood Group","Class","Academic Year","Percentage","Registration No","Date of birth","Birth Place","Email","Address","City","Taluka","District","Pin Code","Parents Mobile No","Student Mobile No","Status","Added On","Last Updated");
			$i=0+1;
			$full_price_total=0;
			$nights_total=0;
			$nr_pax=0;
			foreach($result as $row)
			{
				...
				$get_init_id = $app->cmx->get_field("cm_last_exam","percen_new","reg_id='".$row["id"]."' and exam_name1='3'");
	...
	$fields=array("sr","surname","name","fathername","mothername","gender","cat_name","blood_group","class","acd_year","percen_new","reg_no","dob","birth_place","email","address","city","taluka","district","pin_code","parents_mobile_no","student_mobile_no","status","added_on","last_updated");
