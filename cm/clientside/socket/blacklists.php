<?php 
	...
	while($row = mysql_fetch_array( $rs ))
	{
		...
		$library_hours1 = $row['library_hours'];
		$assignment1 = $row['assignment'];
		$percentage1 = $row['startpercentage'].'-'.$row['endpercentage'];
		...
		$action='<div class="btn-group">
										...
										onclick="javascript:edit_blacklists(\''.$id_enc.'\')"><i class="glyphicon glyphicon-edit"></i> Edit</a></li>
                                          <li><a href="javascript:void(0);"  onclick="javascript:confrim_del_ajax({\'ac_id\':\''.$id_enc.'\', \'connector\':\'delete_blacklists\'})"><i class="glyphicon glyphicon-trash"></i> Delete</a></li>
                                        </ul>
                                      </div>';
...
$html.=" <td><label class=\"custom-checkbox-item\">";
		...
		$html.="<td>{$sr}</td>";
        $html.="<td>{$library_hours1}</td>";
        $html.="<td>{$assignment1}</td>";
        $html.="<td>{$percentage1}</td>";
