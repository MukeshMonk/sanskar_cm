	...
	$record_id=$app->getPostVar("record_id");
	  ...
	  if($name!= NULL)
	  {
		...
		if($record_id=="")
		{
			...
			$obj_model_log = $app->load_model("cm_salutations");
			$obj_model_log->map_fields($update_field);
			$ins=$obj_model_log->execute("INSERT");
			$msg="Salutations Added Successfully.";
		}
		else
		{
			...
			$obj_model_log = $app->load_model("cm_salutations",$record_id);
			$obj_model_log->map_fields($update_field);
			$ins=$obj_model_log->execute("UPDATE");
			
			$msg="Salutations Updated Successfully.";
			
		}
		
		 echo $obj_JSON->encode(array("RESULT"=>"SUCCESS","MSG"=>$msg,"retriver"=>"salutations"));			 	
