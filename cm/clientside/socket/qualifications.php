<?php 
	...
	$sql="SELECT `cm_qualifications`.* FROM `cm_qualifications` WHERE id!=0 and status!=2".$cond.$ord_cond;
	...
	while($row = mysql_fetch_array( $rs ))
	{
		...
		$action='<div class="btn-group">
										...
										onclick="javascript:edit_qualifications(\''.$id_enc.'\')"><i class="glyphicon glyphicon-edit"></i> Edit</a></li>
                                          <li><a href="javascript:void(0);"  onclick="javascript:confrim_del_ajax({\'ac_id\':\''.$id_enc.'\', \'connector\':\'delete_qualifications\'})"><i class="glyphicon glyphicon-trash"></i> Delete</a></li>
                                        </ul>
                                      </div>';
