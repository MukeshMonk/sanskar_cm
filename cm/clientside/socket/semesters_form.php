	...
	$record_id=$app->getPostVar("record_id");
	  ...
	  if($name!= NULL)
	  {
		...
		if($record_id=="")
		{
			...
			$obj_model_log = $app->load_model("cm_semesters");
		...
		else
		{
			...
			$obj_model_log = $app->load_model("cm_semesters",$record_id);
			$obj_model_log->map_fields($update_field);
			$ins=$obj_model_log->execute("UPDATE");
			
			$msg="Semesters Updated Successfully.";
			
		}
		
		 echo $obj_JSON->encode(array("RESULT"=>"SUCCESS","MSG"=>$msg,"retriver"=>"semesters"));			 	
