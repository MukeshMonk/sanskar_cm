<?php 
	...
	$sql="SELECT `cm_users`.*,cm_roles.name FROM `cm_users`

			LEFT JOIN cm_roles ON cm_roles.id = cm_users.role_id WHERE cm_users.id!=0 and cm_users.status!='Trash'".$cond.$ord_cond;
...

	{
		...
		$asses_name=$row['firstname']." ".$row['lastname'];
		...
		$lastupdated=date('d-m-Y',$row['added']);
		...
		$action='<div class="btn-group">
										...
										onclick="javascript:edit_users(\''.$id_enc.'\')"><i class="glyphicon glyphicon-edit"></i> Edit</a></li>

                                          <li><a href="javascript:void(0);"  onclick="javascript:confrim_del_ajax({\'ac_id\':\''.$id_enc.'\', \'connector\':\'delete_users\'})"><i class="glyphicon glyphicon-trash"></i> Delete</a></li>
