<?php
	...
	if(count($delids)>0)
	{
			...
			$obj_model_log = $app->load_model("cm_occupationoffather",$acd_id);
			$obj_model_log->map_fields($update_field);
			$ins=$obj_model_log->execute("UPDATE");
			}
	echo $obj_JSON->encode(array("RESULT"=>"SUCCESS","MSG"=>"Occupation Deleted Successfully.","retriver"=>"occupationoffather"));
	}
	else
	{
