<?php 
	...
	$record_id=$app->getPostVar("record_id");
...

	  {
		...
		{
			$obj_model_log_check = $app->load_model("cm_board_uni");

			$rs_rd=$obj_model_log_check->execute("SELECT",false,"","board_name='".$name."'");

			if(count($rs_rd)>0){
				$msg="Board Already Added.";
				$rslt="ERROR";
			}
			else
			{
				...
				$obj_model_log = $app->load_model("cm_board_uni");
...

		{
			...
			$rs_rd=$obj_model_log_check->execute("SELECT",false,"","board_name='".$name."' and id!='".$record_id."'");
			...
			else
			{
				...
				$obj_model_log = $app->load_model("cm_board_uni",$record_id);
