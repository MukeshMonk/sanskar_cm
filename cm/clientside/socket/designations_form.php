	...
	$record_id=$app->getPostVar("record_id");
	  ...
	  if($name!= NULL)
	  {
		...
		if($record_id=="")
		{
			...
			$obj_model_log = $app->load_model("cm_designations");
		...
		else
		{
			...
			$obj_model_log = $app->load_model("cm_designations",$record_id);
			$obj_model_log->map_fields($update_field);
		 ...
		 echo $obj_JSON->encode(array("RESULT"=>"SUCCESS","MSG"=>$msg,"retriver"=>"designations"));			 	
