					 ...
					 $(document).on('click','.call_export',function()
{
			...
			//	echo $obj_model_admin->sql; exit;
					...
					// $obj_model_sch4 = $this->app->load_model("exam_schedule");
					// $max_mark = $obj_model_sch4->execute("SELECT",false,"SELECT exam_schedule.* FROM `exam_schedule` LEFT JOIN cm_final_term_marks ON exam_schedule.term_id=cm_final_term_marks.term_id  WHERE exam_schedule.cource_id='".$rs_gr[0]['cource_id']."' GROUP BY  exam_schedule.id");
					// //echo 	$obj_model_sch4->sql; exit;
					// $this->app->assign("max_marks",$max_mark);
					 ...
					 $(document).on('click','.call_export',function()
{
			  	...
			  	if($id_dec > 0 && $record_id!="")
				{
					...
					$rs_gr1 = $obj_model_gr1->execute("SELECT",false,"SELECT cm_terms.* FROM cm_terms LEFT JOIN cm_final_term_marks ON cm_terms.id=cm_final_term_marks.term_id WHERE cm_terms.id!=0 AND cm_terms.status!=2 AND cm_terms.assessment_id='".$rs_gr[0]['cource_id']."' GROUP BY cm_terms.id");
