		...
		function onload(){
		  ...
		  if($action=="" || $action=="students")
		  {
			  $inscript.="
	            ...
	            $category_dd=$this->app->cmx->create_dd($class_tbl,$class_condition,$field_one,$field_two,$class_text,$classorder_condition);	
				$this->app->assign("class_dd",$category_dd);
		...
		function save_attendance()
		{
			...
			$comparedatefortodayattandance = "STR_TO_DATE(DATE_FORMAT(FROM_UNIXTIME(add_date), '%Y-%m-%d'),'%Y-%m-%d') = '".date('Y-m-d')."'";
				...
				for($i=0;$i<count($student_id);$i++)
				{
					...
					$student_atten_check=$this->app->cmx->check_student_attendence($student_id[$i],date('Y-m-d'));		
					if(count($student_atten_check)>0)
					{
						if($i == 0)
						{
							$obj_model_log = $this->app->load_model("cm_attendance");
							$obj_model_log->execute("DELETE",false,"delete from cm_attendance where mkattend_id=".$record_id." AND ".$comparedatefortodayattandance." AND default_status NOT IN('Official Leave','Medical Leave')");
						}
...
// 			
		...
		function editreporttable($ExeclHeads,$array_field,$fields,$edit_report_result)
		{
			...
			$html .= "
					...
					{				
						...
						{
							$date=date('Y').'-'.$month.'-'.sprintf('%02d', ($j-3));	
		...
		{			
			...
			foreach($studentarray as $s => $studentarra)
			{
				...
				{
					$sql="update cm_attendance set default_status='".$dateval[$dk][$s]."' where sem='".$semester."' AND course='".$course."' AND mkattend_id='".$mkattend_id."' AND student_id='".$studentarra."' AND add_date='".$dateke."'  ";
