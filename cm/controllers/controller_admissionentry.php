		...
		$('.indatepicker1').val(start.format('DD-MM-YYYY'));
		  ...
		  elseif($action=="enquiries")
		  {
			// $this->app->setAddJS("http://maps.googleapis.com/maps/api/js?key=AIzaSyBDyogimT22Vo14OJJ87IgUFvoHwbKUfME&libraries=places");
			  ...
			  $student_count = $this->app->load_model("cm_enquiries");
			  $total_stu = $student_count->execute("SELECT",false,"SELECT COUNT(id) as stu_id FROM cm_enquiries WHERE acd_year='4' AND status!='Deleted'");
			  $this->app->assign("total_students",$total_stu[0]['stu_id']);	
		  ...
		  elseif($action=="registration")
		  {
			...
			//$this->app->setAddJS("http://maps.googleapis.com/maps/api/js?key=AIzaSyBDyogimT22Vo14OJJ87IgUFvoHwbKUfME&libraries=places");	
			 ...
			 $student_count = $this->app->load_model("cm_register");
			 $total_stu = $student_count->execute("SELECT",false,"SELECT COUNT(id) as stu_id FROM cm_register WHERE cm_academic_year='4' AND status!='Deleted'");
			 $this->app->assign("total_students",$total_stu[0]['stu_id']);	
