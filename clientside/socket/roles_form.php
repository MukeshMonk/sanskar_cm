...
if($_SESSION['Role']==1)
	{
	  if($name!= NULL)
	  {
		...
		if($record_id=="")
		{
			...
			$obj_model_log = $app->load_model("cm_roles");
		...
		else
		{
			...
			$obj_model_log = $app->load_model("cm_roles",$record_id);
			$obj_model_log->map_fields($update_field);
		 ...
		 echo $obj_JSON->encode(array("RESULT"=>"SUCCESS","MSG"=>$msg,"retriver"=>"roles"));			 	
	...
	{
		echo $obj_JSON->encode(array("RESULT"=>"ERROR","MSG"=>"You have not permission to create a new Role"));	
	}
