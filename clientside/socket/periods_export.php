<?php 
	...
	else
	{
	 ...
	 $sql="SELECT `cm_periods`.* FROM `cm_periods` WHERE id!=0 and status!=2".$cond.$ord_cond;

	 $obj_model_hm = $app->load_model("cm_periods");
	 ...
	 if(count($result)>0)
	 {
			...
			//SETUP EXCEL HEADS
			$ExeclHeads=array("Sr.","Display Name","Name","Start Time","End Time","Status","Added On","Last Updated");
	...
	$fields=array("sr","display_name","name","start_time","end_time","status","added_on","last_updated");
		
		$excel_postfix="periods_".time();
