<?php
	...
	$ipp=10;
	$sql="SELECT  `cm_exam_subjects`.*,exam_schedule.acd_year_id,exam_schedule.cource_id,exam_schedule.term_id,cm_subjects.subject_name FROM `cm_exam_subjects` LEFT JOIN exam_schedule ON exam_schedule.id=cm_exam_subjects.exam_id LEFT JOIN cm_subjects ON cm_exam_subjects.sub_id=cm_subjects.id  WHERE exam_schedule.is_attendance='No' and cm_exam_subjects.id!=0 and exam_schedule.status!='2' and cm_exam_subjects.status!='2' ".$cond.$ord_cond; 
