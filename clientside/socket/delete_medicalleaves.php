<?php
	...
	if($acd_id>0)
	{
			...
			$obj_model_log = $app->load_model("cm_medicalleaves",$acd_id);
			$obj_model_log->map_fields($update_field);
			$ins=$obj_model_log->execute("UPDATE");
			
			$obj_model_attandence = $app->load_model("cm_attendance");
			$obj_model_attandence->execute("DELETE",false,"","medical_leave_id=".$acd_id);
			
			
	echo $obj_JSON->encode(array("RESULT"=>"SUCCESS","MSG"=>"Medical Leave Deleted Successfully.","retriver"=>"medicalleaves"));
	}
	else
	{
