<?php 
	...
	else
	{
	 ...
	 $sql="SELECT `cm_medicalleaves`.* FROM `cm_medicalleaves` WHERE id!=0 and status!=2".$cond.$ord_cond;

	 $obj_model_hm = $app->load_model("cm_medicalleaves");
	 ...
	 if(count($result)>0)
	 {
			...
			//SETUP EXCEL HEADS
			$ExeclHeads=array("Sr.","Form","Doctor Name","Start Date","End Date","Student Id","Status","Added On","Last Updated");
	...
	$fields=array("sr","form_no","doctor_name","start_date","end_date","student_idno","status","added_on","last_updated");
		
		$excel_postfix="medicalleaves_".time();
