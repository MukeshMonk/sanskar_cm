<?php
	...
	$ipp=10;
	$sql="SELECT cm_exam_subjects.*,exam_schedule.acd_year_id,exam_schedule.cource_id,exam_schedule.term_id, cm_subjects.subject_name FROM `cm_exam_subjects` LEFT JOIN exam_schedule ON exam_schedule.id=cm_exam_subjects.exam_id LEFT JOIN cm_subjects ON cm_exam_subjects.sub_id=cm_subjects.id group by cm_exam_subjects.sub_id"; 
	...
	$course_name=$row['clss_name'];
		...
		$enter_mark_btn='<a href="'.CMX_ROOT.'/examination/scheduled_marks/add/'.$id_enc.'/"><i class="glyphicon glyphicon-edit"></i> Edit Marks</a>';
